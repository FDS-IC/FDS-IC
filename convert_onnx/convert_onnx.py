...
class ConverttOnnx:
    def __init__(self, args, model, tokenizer):
        self.args = args
        self.device = "cuda:0" if torch.cuda.is_available() else "cpu"  # 自动选择设备
        self.model = model
        self.model.to(self.device)
        self.tokenizer = tokenizer
        self.id2label = args.id2label
...
if __name__ == "__main__":
    ...
    # model.load_state_dict(torch.load(model_path, map_location=torch.device(self.device)), strict=True)
    pprint('Load ckpt from {}'.format(ckpt_path))

    # tokenizer = BertTokenizer.from_pretrained("hf1/chinese-bert-wwm-ext")
    # model = BertNer(args)
